<div class="space-y-6">
    ...
    <form wire:submit="save" class="bg-white rounded-2xl shadow-sm border border-suraki-neutral-dark p-6 md:p-8 space-y-6">
            ...
            <div class="border-t border-gray-200 pt-6 mt-6">
                ...
                <div class="space-y-4">
                    ...
                    @foreach($days as $key => $label)
                        <div class="flex items-center gap-4 p-4 rounded-xl {{ !empty($schedule[$key]['start']) ? 'bg-blue-50 border border-blue-100' : 'bg-gray-50 border border-gray-100' }}">
                            <div class="w-24 font-bold text-gray-700">{{ $label }}</div>
                            <div class="flex-1 grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-semibold text-gray-500 mb-1">Entrada</label>
                                    <input type="time" wire:model="schedule.{{ $key }}.start" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-suraki-primary focus:ring-suraki-primary text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-gray-500 mb-1">Salida</label>
                                    <input type="time" wire:model="schedule.{{ $key }}.end" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-suraki-primary focus:ring-suraki-primary text-sm">
                                </div>
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="pt-4 flex justify-end">
            <button type="submit" class="bg-suraki-primary hover:bg-suraki-secondary text-white font-bold py-3 px-8 rounded-xl shadow-md hover:shadow-lg transition-all flex items-center gap-2">
